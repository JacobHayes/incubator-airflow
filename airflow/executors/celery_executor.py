...
class CeleryExecutor(BaseExecutor):
    ...
    def sync(self):
        ...
        for key, async_ in list(self.tasks.items()):
            try:
                state = async_.state
                if self.last_state[key] != state:
                    if state == celery_states.SUCCESS:
                        ...
                        self.log.info("Unexpected state: %s", async_.state)
                    self.last_state[key] = async_.state
    ...
    def end(self, synchronous=False):
        if synchronous:
            while any([
                    async_.state not in celery_states.READY_STATES
                    for async_ in self.tasks.values()]):
                time.sleep(5)
        self.sync()
